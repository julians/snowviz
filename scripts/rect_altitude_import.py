...
def main(argv):
    ...
    for item in data["data"]:
        cur.execute("""INSERT INTO altitude_rects (altitude, the_geom) VALUES (%s, ST_GeomFromText(%s, 4326))""", (item[8], 'POLYGON((%s %s, %s %s, %s %s, %s %s, %s %s))' % (item[0], item[1], item[2], item[3], item[4], item[5], item[6], item[7], item[0], item[1])))
